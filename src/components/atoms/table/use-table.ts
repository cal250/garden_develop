...
interface Props<T> extends HTMLNextUIProps<"table"> {
  ...
  /**
   * Whether to hide the spacer between the heading and the table.
   */
  hideHeadingSpacer?: boolean;
}
...
export function useTable<T extends object>(originalProps: UseTableProps<T>) {
  ...
  const { gridProps } = useReactAriaTable<T>(
    { ...otherOriginalProps },
    state,
    domRef,
  );
  ...
  const getTableProps: PropGetter = useCallback(
    (props) => ({
      ...
      className: slots.table({
        class: clsx(classNames?.table, props?.className),
      }),
    }),
