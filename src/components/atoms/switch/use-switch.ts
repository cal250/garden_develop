...
import { ReactNode, Ref, useCallback, useId, useMemo, useRef } from "react";
...
export function useSwitch(originalProps: UseSwitchProps = {}) {
  ...
  const { inputProps, isPressed, isReadOnly } = useReactAriaSwitch(
    ariaSwitchProps,
    state,
    inputRef,
  );
  ...
  const getStartContentProps = useCallback<PropGetter>(
    (props = {}) => ({
      ...
      className: slots.startContent({
        class: clsx(classNames?.startContent, props?.className),
      }),
    }),
