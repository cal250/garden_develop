...
export interface TagsInputProps extends Pick<FlowerPickerProps, 'onSelectionChange'> {
  selectedTag: string[]
}

export default function TagsInput({ selectedTag, onSelectionChange }: TagsInputProps) {
  ...
  const { mutate: addTag, isPending: isAddingTag } = useMutation({
    ...
    onSuccess: () => {
      const capitalizedTag = customTag.charAt(0).toUpperCase() + customTag.slice(1);
      onSelectionChange?.(new Set([capitalizedTag]))
      setCustomTag('')
