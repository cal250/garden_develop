...
import { Key, ReactNode, RefObject, useCallback, useEffect, useMemo, useRef } from 'react'
...
export function useSelect<T extends object>(originalProps: UseSelectProps<T>) {
  ...
  const {
    ...
    state,
    triggerRef as RefObject<any>,
  )

  const isInvalid = originalProps.isInvalid || validationState === 'invalid' || isAriaInvalid

  const { isPressed, buttonProps } = useAriaButton(triggerProps, triggerRef as RefObject<any>)
