...
const CreativityCompostFertilizeDropdown = () => {
  const [dropdownOpen, setDropdownOpen] = useState(false)
  const [selected, setSelected] = useState('symbols')
  const options = ['symbols', 'arrows', 'words']
      ...
      <div className="max-w-xl w-full bg-[#2D1B3A] rounded-3xl shadow-lg p-8 flex flex-col items-center">
        ...
        <div className="w-full bg-[#3E2A55] rounded-xl p-6 mb-6 flex flex-col items-center">
          ...
          <div className="flex gap-2 mb-6">
            <Button
              variant="solid"
              className="bg-[#825FA3] text-white relative"
              onClick={() => setDropdownOpen(!dropdownOpen)}
            >
              {selected}
              {dropdownOpen && (
                <div className="absolute left-0 top-full mt-2 w-32 bg-white rounded shadow-lg z-10">
                  {options.map((option) => (
                    <div
                      key={option}
                      className="px-4 py-2 hover:bg-[#F7D046] cursor-pointer text-[#2D1B3A]"
                      onClick={() => {
                        setSelected(option)
...
export default CreativityCompostFertilizeDropdown
