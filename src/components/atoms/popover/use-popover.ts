...
import { ReactAriaPopoverProps, useReactAriaPopover } from "./use-aria-popover";
...
export function usePopover(originalProps: UsePopoverProps) {
  ...
  const getContentProps = useCallback<PropGetter>(
    (props = {}) => ({
      ...
      "data-open": dataAttr(state.isOpen),
      "data-arrow": dataAttr(showArrow),
      "data-placement": getArrowPlacement(
        ariaPlacement || "top",
        placementProp,
      ),
  ...
  const onPress = useCallback(
    (e: PressEvent) => {
      ...
      if (
        e.pointerType === "touch" &&
        (originalProps?.backdrop === "blur" ||
          originalProps?.backdrop === "opaque")
      ) {
