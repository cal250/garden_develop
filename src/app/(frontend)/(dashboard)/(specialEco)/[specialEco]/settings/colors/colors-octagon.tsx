...
export default function ColorsOctagon({
  cocoon,
  wedgeIndex = -1,
  onSegmentClick,
}: ColorsOctagonProps) {
  ...
  const iconUrl = typeof cocoon?.icon === 'string' ? cocoon.icon : (cocoon?.icon?.url ?? '')
...
interface ColorsOctagonProps extends Pick<RadialPolygonProps, 'onSegmentClick'> {
  wedgeIndex?: number
  cocoon: Cocoon
}
