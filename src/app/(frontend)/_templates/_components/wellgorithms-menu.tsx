        ...
        {selectedItem ? (
          <button className="text-color-2 flex gap-2 items-center">
            <div className="flex flex-col justify-center gap-0">
              <span className="text-[20px] font-bold leading-[22px]">wellgorithms</span>
              <span className="text-[12px] font-bold leading-[14px]">{selectedItem.label}</span>
            </div>
            <selectedItem.icon className="size-[20px]" />
          </button>
        ) : (
          <button className="text-[20px] font-bold">wellgorithms</button>
        )}
      </PopoverTrigger>
      <PopoverContent
        polygon={Rectagon}
        ...
        >
          {menu.map((item) => (
            <MenuItem
              ...
              onPress={() => {
                setOpen(false)
                props.onChange(item.key as Wellgorithm)
              }}
...
interface WellgorithmsMenuProps {
  wellgorithm?: Wellgorithm
  onChange: (wellgorithm: Wellgorithm) => void
}
