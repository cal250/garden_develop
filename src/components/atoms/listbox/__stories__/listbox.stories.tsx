...
import { Listbox, ListboxItem, ListboxProps, ListboxSection } from "../index";
...
const PullRequestIcon = (props: any) => (
  <svg
    height="1em"
    viewBox="0 0 24 24"
    width="1em"
...
const WithTopContentTemplate = ({ color, ...args }: any) => {
  ...
  const topContent = React.useMemo(() => {
    ...
    return (
      <ScrollShadow
        hideScrollBar
        className="flex w-full gap-1 bg-background px-2 py-0.5"
...
const WithBottomContentTemplate = ({ color, ...args }: any) => {
  const [values, setValues] = React.useState<Selection>(
    new Set(["1", "3", "5"]),
  );
...
const LargeDatasetTemplate = (args: ListboxProps & { numItems: number }) => {
  ...
  return (
    <div className="flex w-full max-w-full px-20 py-20">
