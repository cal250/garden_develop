...
import { AiHumometer } from '@/app/(frontend)/_templates/create/sprout/_components/ai-humometer'
...
const Page: React.FC<Props> = (props) => {
  ...
  const [humometerValue, setHumometerValue] = useState<number>()
  ...
  return (
    <section className="mt-[-44px] flex flex-col items-center gap-[30px]">
      ...
      <div className="mt-[130px] flex flex-col gap-20 items-center ">
        <Rectagon
        ...
        >
          <Trapezoid
            ...
            strokeWidth={0}
          >
            <span>AI humometer</span>
          </Trapezoid>
          <span className="text-[20px] font-bold">
            how much is your writing, and how much from AI?
          </span>
          <div className="w-[873px] pb-24">
            <AiHumometer
              onValueChange={setHumometerValue}
              parameters={[
                'big ideas',
                'main points',
                'facts and flourishes',
                'editing',
                'final polish',
              ]}
            />
            <RegularPolygon
              sides={8}
              className="size-[100px] bg-[#F6AA22] absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 text-[48px] font-bold text-white"
              stroke="rgb(var(--color-4))"
            >
              <span>{humometerValue ? humometerValue : '-'}</span>
            </RegularPolygon>
          </div>
        </Rectagon>
        <Button
