  ...
  segments: [
    {
      content: (
        <span
          className={`text-[17px] font-bold ${index === 7 ? 'text-[#FF84C7]' : 'text-color-2'}`}
        >
          {value}
        </span>
      ),
    ...
    {
      content: value ? index === 7 ? <PinkFlowerIcon /> : <YellowFlowerIcon /> : '',
    },
  ],
  boundary: {
    inner: {
...
const WordPageBanner: React.FC<Props> = (props) => {
  return (
    <OctagonBannerLayout>
      <RadialPolygon
        sides={8}
